...
export class AdventureClient {
  ...
  constructor({ serviceURL, endpoint }) {
    // Note: Authentication should be handled server-side only
    // If NEXT_PUBLIC_AEM_AUTH is set, it will be exposed to client
    // Consider using server-side only env vars and proxy through API
    const auth = process.env.AEM_AUTH 
      ? process.env.AEM_AUTH.split(':')
      : undefined;
